    ...
    def start_analysis(self):
        ...
        if not self.is_running:
            self.analyzer = CompleteAnalyzer(
                ...
                save_interval=10,
                record_interval=60,  # 每分钟生成一次分析记录
                auto_record=True
            )
        ...
        if self.is_running:
            ...
            if self.analyzer:
                try:
                    ...
                    if hasattr(self.analyzer, 'persistent_analyzer'):
                        self.analyzer.persistent_analyzer.end_session()
                    
                    # 不要关闭analyzer，保留引用以便后续访问分析记录
                    # 但要停止分析处理
                    self.analyzer.stop()
                    
                    logger.info(f"用户 {self.username} 停止分析")
                except Exception as e:
                    logger.error(f"停止分析器失败: {e}")
            else:
                logger.warning(f"用户 {self.username} 停止分析时分析器为空")
...
class WebApp:
    ...
    def get_db(self) -> DatabaseManager:
        """获取数据库连接，如果连接已关闭则重新创建"""
        try:
            # 尝试执行一个简单查询来测试连接是否有效
            self.db.get_sessions(limit=1)
            return self.db
        except Exception as e:
            logger.warning(f"数据库连接已关闭，重新创建: {e}")
    ...
    def _setup_routes(self):
        ...
        @self.app.post("/api/create-record/{user_id}")
        async def create_analysis_record(user_id: str, record_name: str = None):
            """创建分析记录"""
            if user_id not in self.user_sessions:
                raise HTTPException(status_code=404, detail="用户不存在")
            
            session = self.user_sessions[user_id]
            if not session.is_running or not session.analyzer:
                raise HTTPException(status_code=400, detail="分析未运行，无法创建新记录")
            
            try:
                record_id = session.analyzer.create_analysis_record(
                    custom_name=record_name or f"手动记录_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
                    additional_data={"created_by": "web_interface", "manual": True}
                )
                
                return {
                    "success": True,
                    "record_id": record_id,
                    "message": f"成功创建分析记录 (ID: {record_id})"
                }
            except Exception as e:
        ...
        async def get_analysis_records(user_id: str, limit: int = Query(20, ge=1, le=100)):
            ...
            try:
                if session.analyzer:
                    # 如果分析器存在，使用分析器获取记录
                    records = session.analyzer.get_analysis_records(limit=limit)
                else:
                    # 如果分析器不存在，直接从数据库获取记录
                    # 需要获取会话ID
                    db = self.get_db()
                    session_id = None
                    for s_id in db.get_sessions(limit=100):
                        if s_id.get('session_name', '').startswith(f"{session.username}_"):
                            session_id = s_id.get('id')
                            break
                    
                    if not session_id:
                        return {"records": []}
                    
                    records = db.get_analysis_records(session_id, limit)
        ...
        @self.app.get("/api/record/{user_id}/{record_id}")
        async def get_analysis_record_detail(user_id: str, record_id: int):
            """获取分析记录详情"""
            if user_id not in self.user_sessions:
                raise HTTPException(status_code=404, detail="用户不存在")
            ...
            try:
                # 获取记录详情
                if session.analyzer:
                    db = session.analyzer.persistent_analyzer.db
                else:
                    db = self.get_db()
        ...
        @self.app.post("/api/export-record/{user_id}")
        async def export_analysis_record(user_id: str, record_id: int = None):
            """导出分析记录"""
            if user_id not in self.user_sessions:
                raise HTTPException(status_code=404, detail="用户不存在")
            ...
            try:
                if session.analyzer:
                    # 如果分析器存在，使用分析器导出记录
                    filepath = session.analyzer.export_analysis_record(record_id=record_id)
                else:
                    # 如果分析器不存在，手动导出记录
                    db = self.get_db()
                    if record_id is None:
                        ...
                        if not session_id:
                            raise HTTPException(status_code=404, detail="未找到相关会话记录")
                        
                        records = db.get_analysis_records(session_id, 1)
                        if not records:
                    ...
                    # 生成导出文件路径
                    export_dir = "data/analysis_records"
                    os.makedirs(export_dir, exist_ok=True)
                    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                    record_name = record.get('record_name', '').replace(' ', '_')
                    filepath = os.path.join(export_dir, f"record_{record_name}_{timestamp}.json")
                ...
                if not filepath:
                    raise HTTPException(status_code=500, detail="导出分析记录失败")
                
                return {
                    "success": True,
                    "filepath": filepath,
                    "download_url": f"/download/record?path={filepath}",
                    "message": "分析记录导出成功"
                }
            except HTTPException:
                raise
        ...
        @self.app.get("/download/record")
        async def download_record_file(path: str):
            """下载分析记录文件"""
            try:
        ...
        <body>
            ...
            <script>
                ...
                recordsBtn.onclick = function() {
                    if (currentUserId) {
                        window.open(`/static/records.html?user_id=${currentUserId}`, '_blank');
                    }
                };
                ...
                let adaptiveQuality = 0.8;
                let adaptiveScale = 0.9;
                ...
                function captureFrame() {
                    try {
                        ...
                        if (isProcessing) {
                            ...
                            if (frameSkipCount > 3) {
                                // 动态降低质量和分辨率
                                adaptiveQuality = Math.max(0.7, adaptiveQuality - 0.02);
                                adaptiveScale = Math.max(0.8, adaptiveScale - 0.02);
                        ...
                        // 恢复质量和分辨率
                        adaptiveQuality = Math.min(0.9, adaptiveQuality + 0.01);
                        adaptiveScale = Math.min(1.0, adaptiveScale + 0.01);
